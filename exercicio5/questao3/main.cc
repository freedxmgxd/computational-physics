...
int main() {
  ...
  // error = abs(integral_by_wolfram - result);
  // dataFile << "\"Trapezoidal 1000\" " << 1000 << " " << result << " " <<
  // error
  //          << "\n";
  ...
  // cout << "Adaptativo: " << simpson_adaptativo(func, a, b, 2, epsilon) <<
  // endl;

  // // e) Integração de Romberg
  std::ofstream dataFile("results.dat");
  for (auto i = 0; i <= 30; i++) {
    cout << "E(" << i / 10.0 << ")" << romberg(func, a, i / 10.0, epsilon)
         << endl;
    dataFile << "\"Romberg " << i / 10.0 << "\" " << i / 10.0 << " "
             << romberg(func, a, i / 10.0, epsilon) << " "
             << abs(0.5 - romberg(func, a, i / 10.0, epsilon)) << "\n";
  }
  ...
  gnuplotScript << "set ylabel 'E(x)'\n";
  gnuplotScript << "set style data linespoints\n";
  gnuplotScript << "set key outside\n";
  gnuplotScript << "plot 'results.dat' using 2:3 title 'E(x)' with linespoints, \n";
  gnuplotScript.close();
